...
const { nsfw, weeaboo, downloader, sticker, fun, misc, toxic, cekResi } = require('../lib')
...
module.exports = msgHandler = async (bocchi = new Client(), message) => {
    try {
        ...
        const isSimiOn = simi.includes(chatId,groupId) || false
        ...
        // Simi-simi function
		    if ((!isCmd && isSimiOn) && message.type === 'chat') {
        ...
        switch (command) {
            ...
            case 'listrik':
                    ...
                    misc.listrik(q)
                        .then(async({ data }) => {
                            await bocchi.sendText(from,  `*「 Info Tagihan 」*\n\n${data.replace('Jenis Tagihan','JenisTagihan')}`,id)
                        })
                        .catch(async(err) => {
                            console.error(err)
                            await bocchi.reply(from, 'Error!', id)
            ...
            case 'simisimi':
                ...
                case 'simi':
                    ...
                    if (args[0] == 'on') {
                    if (isSimiOn) return await bocchi.reply(from, `Command simi sudah diaktifkan sebelumnya.`, id)
                        simi.push(chatId, groupId)
                        fs.writeFileSync('./database/bot/simi.json', JSON.stringify(simi))
                        bocchi.reply(from, 'Mengaktifkan bot simi-simi!', id)
                    } else if (args[0] == 'off') {
                            let inxx = simi.indexOf(chatId, groupId)
                            simi.splice(inxx, 1)
                            fs.writeFileSync('./database/bot/simi.json', JSON.stringify(simi))
            ...
            case 'tos':
                    if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    await bocchi.sendText(from, ind.tos())
                    break
            case 'shortlink':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!isUrl(url)) return await bocchi.reply(from, ind.wrongFormat(), id)
            ...
            case 'igstalk':
                ...
                misc.igStalk(q)
                    .then(async ({ result }) => {
                        if (result === null) {
                            await bocchi.reply(from, 'Username Tidak ditemukan!', id)
                        } else {
            ...
            case 'resi':
                    if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (args.length !== 2) return bocchi.reply(from, `Maaf, format pesan salah.\nSilahkan ketik pesan dengan ${prefix}resi <kurir> <no_resi>\n\nKurir yang tersedia:\njne, pos, tiki, wahana, jnt, rpx, sap, sicepat, pcp, jet, dse, first, ninja, lion, idl, rex`, id)
                    const kurirs = ['jne', 'pos', 'tiki', 'wahana', 'jnt', 'rpx', 'sap', 'sicepat', 'pcp', 'jet', 'dse', 'first', 'ninja', 'lion', 'idl', 'rex']
                    if (!kurirs.includes(args[0])) return bocchi.sendText(from, `Maaf, jenis ekspedisi pengiriman tidak didukung layanan ini hanya mendukung ekspedisi pengiriman ${kurirs.join(', ')} Tolong periksa kembali.`)
                    console.log('Memeriksa No Resi', args[1], 'dengan ekspedisi', args[0])
                    cekResi(args[0], args[1]).then((result) => bocchi.sendText(from, result))
                    break
            case 'receipt':
            case 'resep':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
